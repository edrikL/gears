...
function main() {
  ...
  global.mapZoom = 15;
  updateTime();
  global.siteUrl = "http://code.google.com/apis/gears/samples/running_man/step7/index.html";
  initDB();
  installShortcut();
}
...
function installShortcut() {
  if (getPreference('Shortcut') == false) {
    ...
    desktop.createShortcut("RunningMan", global.siteUrl,
      { "48x48" : "../images/icon.png" }, "RunningMan Step7");
...
function on_journeys() {
  ...
  while (rs.isValidRow()) {
   ...
   if (description == null) {
    ...
    html += "<div class='rowDesc' onclick='showMap(" + rowID + ")'>" + description + "</div>";
